...
(function () {
  ...
  function select(id) {
    ...
    if (selected) {
      ...
      // Jump rather than animate. The distance from the plan to the panel is
      // most of a screen and grows with the page, and a long smooth scroll is
      // slow to sit through and easy to lose your place in. Jumping also means
      // there is no motion to have to reduce.
      if (open && open.scrollIntoView) {
        try {
          open.scrollIntoView({ behavior: "auto", block: "start" });
        } catch (e) {
